...
def proj_ref_src(src_fea: torch.Tensor, src_proj: torch.Tensor, ref_proj: torch.Tensor, depth_samples: torch.Tensor, return_uv=False):
    ...
    with torch.no_grad():
        ...
        rot_depth_xyz = torch.matmul(rot, xyz).unsqueeze(2).repeat(1, 1, num_depth, 1) * depth_samples.view(batch, 1, num_depth, height * width)  # [B, 3, Ndepth, H*W]
        ...
        grid_uv = proj_xyz[:, :2, :, :] / proj_xyz[:, 2:3, :, :]  # [B, 2, Ndepth, H*W]
        
        proj_x_normalized = grid_uv[:, 0, :, :] / ((width - 1) / 2) - 1  # [B, Ndepth, H*W]
        proj_y_normalized = grid_uv[:, 1, :, :] / ((height - 1) / 2) - 1
        grid = torch.stack((proj_x_normalized, proj_y_normalized), dim=3)  # [B, Ndepth, H*W, 2]
        import pdb;pdb.set_trace()
        return None,None


def differentiable_warping(src_fea: torch.Tensor, src_proj: torch.Tensor, ref_proj: torch.Tensor, depth_samples: torch.Tensor, return_uv=False):
    ...
    """
    ishalf = src_fea.dtype is torch.float16
    src_fea = src_fea.float()
    batch, num_depth, height, width = depth_samples.shape
    dtype = src_fea.dtype
    channels = src_fea.shape[1]
    with torch.no_grad():
        proj = torch.matmul(src_proj, torch.inverse(ref_proj))
        if src_fea.dtype is torch.float16:
            proj = proj.half()
        rot = proj[:, :3, :3]  # [B,3,3]
        trans = proj[:, :3, 3:4]  # [B,3,1]
        y, x = torch.meshgrid([ torch.arange(0, height, dtype=dtype, device=src_fea.device),
                                torch.arange(0, width, dtype=dtype, device=src_fea.device),])
        y, x = y.contiguous().view(height * width), x.contiguous().view(height * width)
        xyz = torch.unsqueeze(torch.stack((x, y, torch.ones_like(x))), 0).repeat(batch, 1, 1)  # [B, 3, H*W]
        # import pdb;pdb.set_trace()
        rot_depth_xyz = torch.matmul(rot, xyz).unsqueeze(2).repeat(1, 1, num_depth, 1) * depth_samples.view(batch, 1, num_depth, height * width)  # [B, 3, Ndepth, H*W]
    ...
    if ishalf:
        src_fea_sample,grid_uv = src_fea_sample.half(),grid_uv.half()
    if return_uv:
        return src_fea_sample, grid_uv.view(batch, 2, num_depth, height, width)
    else:
        return src_fea_sample
