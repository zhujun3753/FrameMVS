...
class DepthInitialization(nn.Module):
    ...
    def forward(
        ...
        if init_depth is not None:
            batch = init_depth.shape[0]
            dtype = init_depth.dtype
            # import pdb;pdb.set_trace()
            y_grid, x_grid = torch.meshgrid([torch.arange(0, height, dtype=dtype, device=init_depth.device), torch.arange(0, width, dtype=dtype, device=init_depth.device),])
            y_grid, x_grid = y_grid.contiguous().view(height * width), x_grid.contiguous().view(height * width)
            xy = torch.stack((x_grid, y_grid))  # [2, H*W]
            del x_grid
            del y_grid
            xy = torch.unsqueeze(xy, 0).repeat(batch, 1, 1).view(batch,2,height, width)  # [B, 2, H, W]
            ...
            surrounding_depth = F.grid_sample(init_depth, grid, mode="nearest", padding_mode="reflection", align_corners=True).view(batch, -1, height, width).float()
            depth_sample =torch.arange(-self.patchmatch_num_sample // 2, self.patchmatch_num_sample // 2, 1, device=device).view(1, self.patchmatch_num_sample, 1, 1).repeat(batch_size, 1, height, width).float()
            new_depth_near = surrounding_depth.repeat(1, self.patchmatch_num_sample, 1, 1) * (1 + depth_sample/(self.patchmatch_num_sample // 2)*0.1) 
            #* torch.Size([1, 16, 128, 160])
            new_depth_far = surrounding_depth.repeat(1, self.patchmatch_num_sample, 1, 1) * (1 + depth_sample/(self.patchmatch_num_sample // 2)*0.7)
            new_depth = torch.cat([new_depth_near,new_depth_far],dim=1) #* torch.Size([1, 32, 128, 160])
            return new_depth
            import pdb;pdb.set_trace()
        ...
        if is_empty(depth):
            ...
            # [B,Ndepth,H,W]
            depth_sample = torch.rand(size=(batch_size, patchmatch_num_sample, height, width), device=device
                ) + torch.arange(start=0, end=patchmatch_num_sample, step=1, device=device).view(1, patchmatch_num_sample, 1, 1)
...
class SparseDepthInitialization(nn.Module):
    """Initialization Stage Class"""
    ...
    def __init__(self, patchmatch_num_sample: int = 1) -> None:
        ...
        Args:
            patchmatch_num_sample: number of samples used in patchmatch process
        """
        super(SparseDepthInitialization, self).__init__()
        self.patchmatch_num_sample = patchmatch_num_sample
    ...
    def forward(
        ...
        if is_empty(depth):
            ...
            # other Patchmatch, local perturbation is performed based on previous result
            # uniform samples in an inversed depth range
            depth_sample = (torch.arange(-self.patchmatch_num_sample // 2, self.patchmatch_num_sample // 2, 1, device=device)
...
class Evaluation(nn.Module):
    ...
    def forward(
        ...
        for src_feature, src_proj in zip(src_features, src_projs):
            warped_feature = differentiable_warping(src_feature, src_proj, ref_proj, depth_sample).view(batch, self.G, feature_channel // self.G, num_depth, height, width)
    ...
    def forward(
        ...
        for iter in range(1, self.patchmatch_iteration + 1):
            ...
            weight = depth_weight(
                ...
                neighbors=self.evaluate_neighbors,) * feature_weight.unsqueeze(1)
