...
# from utils import plot,depth2ply,plt_select_mask,get_offset_grid,patch_grid_sample,cal_gray_grad,cal_feature_img
from utils import *
import matplotlib.pyplot as plt
import numpy as np
import matplotlib.cm as cm
from datasets.data_io import read_image
...
class PatchmatchNet(nn.Module):
	...
	def forward(
		...
		#* sparse_depth.shape torch.Size([1, 1, 1024, 1280])
		init_depth = None
		...
		ref_image = images[0] #* torch.Size([1, 3, 1024, 1280])
		...
		for stage in range(self.stages - 1, 0, -1):
			...
			if stage == 3:
				depths, score, view_weights = self.patchmatch_3(
					ref_feature=ref_feature[stage], #* torch.Size([1, 64, 128, 160])
				...
				depths, score, view_weights = self.patchmatch_2(
					ref_feature=ref_feature[stage], #* torch.Size([1, 32, 256, 320])
					src_features=src_features_l,
					ref_proj=ref_proj,
					src_projs=src_proj,
					depth_min=depth_min,
				...
				depths, score, view_weights = self.patchmatch_1(
					ref_feature=ref_feature[stage], #* torch.Size([1, 16, 512, 640])
					src_features=src_features_l,
					ref_proj=ref_proj,
					src_projs=src_proj,
					depth_min=depth_min,
...
def adjust_image_dims(images: List[torch.Tensor], intrinsics: torch.Tensor) -> Tuple[List[torch.Tensor], torch.Tensor, int, int]:
