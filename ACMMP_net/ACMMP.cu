...
__global__ void RandomInitialization(cudaTextureObjects *texture_objects, Camera *cameras, float4 *plane_hypotheses,  float4 *scaled_plane_hypotheses, float *costs,  float *pre_costs,  curandState *rand_states, unsigned int *selected_views, float4 *prior_planes, unsigned int *plane_masks, const PatchMatchParams params, float * init_depth)
{
    ...
    if (!params.geom_consistency && !params.hierarchy && !params.planar_prior && ! params.init_depth_flag)
    {
    ...
    else if(params.init_depth_flag && !params.planar_prior)
    {
        if (init_depth[center] > 0)
        {
            float perturbation = 0.06f;
            float depth_perturbed = init_depth[center];
            ...
            float4 plane_hypothesis_perturbed = GenerateRandomNormal(cameras[0], p, &rand_states[center], depth_perturbed);
...
__global__ void find_edge(float * depths_cuda, float * depth_edge_mask_cuda, int width, int height)
{
    const int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    if (p.x >= width || p.y >= height)
    {
    ...
    const int center = p.y * width + p.x;
    if (depths_cuda[center] > 0)
    {
        // depth_edge_mask_cuda[center] = 1.0;
        int radius = 1;
        float min_depth = 100.0;
        float max_depth = -1.0;
        for(int i=0;i<2*radius;i++)
        {
            int2 p1 = make_int2(p.x+i-radius, p.y+i-radius);
            if (p1.x >= width || p1.y >= height || p1.x<0 || p1.y<0)
            {
                continue;
            }
            const int center1 = p1.y * width + p1.x;
            if(depths_cuda[center1] > 0)
            {
                min_depth = (min_depth > depths_cuda[center1]) ? depths_cuda[center1] : min_depth;
                max_depth = (max_depth < depths_cuda[center1]) ? depths_cuda[center1] : max_depth;
            }
        }
        if(max_depth-min_depth>0.5) //* 不会用到初始值
        {
            depth_edge_mask_cuda[center] = 1.0;
        }
    }
...
void find_edge_cuda(const Camera camera, float * depths_cuda, float * depth_edge_mask_cuda, float * depth_edge_mask_host)
{
    ...
    printf("find_edge_cuda test, width%d, height:%d\n", width, height);
    ...
    find_edge<<<grid_size_randinit, block_size_randinit>>>(depths_cuda, depth_edge_mask_cuda, width, height);
    CUDA_SAFE_CALL(cudaDeviceSynchronize());

    cudaMemcpy(depth_edge_mask_host, depth_edge_mask_cuda, sizeof(float) * width * height, cudaMemcpyDeviceToHost);
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
...
void RunPatchMatch_cuda(std::vector<Camera> cameras, cudaTextureObjects *texture_objects_cuda, Camera *cameras_cuda, float4 *plane_hypotheses_cuda,  float4 *scaled_plane_hypotheses_cuda, float *costs_cuda,  float *pre_costs_cuda,  curandState *rand_states_cuda, unsigned int *selected_views_cuda, float4 *prior_planes_cuda, unsigned int *plane_masks_cuda, const PatchMatchParams params, cudaTextureObjects *texture_depths_cuda, float4 * plane_hypotheses_host, float * costs_host, float * depths_cuda)
{
    ...
    RandomInitialization<<<grid_size_randinit, block_size_randinit>>>(texture_objects_cuda, cameras_cuda, plane_hypotheses_cuda, scaled_plane_hypotheses_cuda, costs_cuda, pre_costs_cuda, rand_states_cuda, selected_views_cuda, prior_planes_cuda, plane_masks_cuda, params, depths_cuda);
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
