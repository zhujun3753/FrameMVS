...
void KD_TREE::multi_thread_rebuild(){
    ...
    // printf("multi_thread_rebuild return \n");
    // return;
    // getchar();
    // int iter_num=0;
    while (!terminated){
        ...
        // printf("rebuild_ptr_mutex_lock\n");
        pthread_mutex_unlock(&rebuild_ptr_mutex_lock);         
        pthread_mutex_lock(&termination_flag_mutex_lock);
        terminated = termination_flag;
        pthread_mutex_unlock(&termination_flag_mutex_lock);
        usleep(100); 
        // printf("iter_num: %d end\n",iter_num++);
...
{
    MANUAL_HEAP q(2*k_nearest); //* 保证无论是pop还是push，第一个值总是最值，可以设置最大或者最小！！！牛皮
    q.clear();
    ...
    // cout<<"k_nearest: "<<k_nearest<<" int(q.size()): "<<int(q.size())<<endl; //* 5 5
    ...
    for (int i=0;i < k_found;i++){
        ...
        // cout<<"q.top().dist: "<<q.top().dist<<endl;
        q.pop();
    }
    // exit(1);
    return;
...
int KD_TREE::Add_Points(PointVector & PointToAdd, bool downsample_on){
    ...
    bool downsample_switch = downsample_on && DOWNSAMPLE_SWITCH; //* 1
    // cout<<"downsample_switch: "<<downsample_switch<<endl;
    // cout<<"NewPointSize: "<<NewPointSize<<endl; //* 1084
    // cout<<"downsample_size: "<<downsample_size<<endl; //* 0.3
    // cout<<"Root_Node->TreeSize: "<<Root_Node->TreeSize<<endl; //* 0.3
    ...
    for (int i=0; i<PointToAdd.size();i++){
        if (downsample_switch){
            ...
            //* 寻找在当前点所在的正方体中的点，保存到 Downsample_Storage
            //* 只是单纯地取点，并没有做什么标记!
            Search_by_range(Root_Node, Box_of_Point, Downsample_Storage);
            //* 在正方体中找与中心点最近的点，即在当前点与Downsample_Storage中的点中寻找与中心点最近的点
...
int KD_TREE::Delete_by_range(KD_TREE_NODE ** root,  BoxPointType boxpoint, bool allow_rebuild, bool is_downsample){   
    ...
    //* 正方体包含当前节点对应的树,则直接将当前树标记为树删除和点删除，并标记需要向下传播,无效点数即为树的点数
    //* 如果这个删除是下采样触发的，则同时标记为树下采样删除和点下采样删除，下采样删除点树为树的点数
    ...
    //* 如果当前节点对应的树全被标记为删除或者没有被标记，就不需要往下处理了，下面的处理主要是涉及当前节点对应的树部分被改变，部分没有
    //* 如果当前节点所在的点在正方形内，则标记为点删除，如果下采样出发则标记为点下采样删除
...
void KD_TREE::Search(KD_TREE_NODE * root, int k_nearest, PointType point, MANUAL_HEAP &q, double max_dist){
    if (root == nullptr || root->tree_deleted) return;   
    //* 计算搜索点到当前节点对应的树所在的长方体的距离，长方体内则值为0
    double cur_dist = calc_box_dist(root, point);
    if (cur_dist > max_dist) return;
    // cout<<"max_dist: "<<max_dist<<endl; //* max_dist: inf
    // exit(1);
    //* 状态向下传播
    int retval;
...
void KD_TREE::Search_by_range(KD_TREE_NODE *root, BoxPointType boxpoint, PointVector & Storage){
    ...
    if (boxpoint.vertex_max[1] <= root->node_range_y[0] || boxpoint.vertex_min[1] > root->node_range_y[1]) return;
    if (boxpoint.vertex_max[2] <= root->node_range_z[0] || boxpoint.vertex_min[2] > root->node_range_z[1]) return;
    //* 当前点所在的下采样区域包含当前树所在区域
    if (boxpoint.vertex_min[0] <= root->node_range_x[0] && boxpoint.vertex_max[0] > root->node_range_x[1] && 
        boxpoint.vertex_min[1] <= root->node_range_y[0] && boxpoint.vertex_max[1] > root->node_range_y[1] && 
        boxpoint.vertex_min[2] <= root->node_range_z[0] && boxpoint.vertex_max[2] > root->node_range_z[1])
    {
        flatten(root, Storage, NOT_RECORD); //* 获得当前树的所有点放入 Storage 中
        return;
    }
    //* 如果当前点所在的下采样区域包含当前树的跟节点处的点，且该点没有被标记为点删除，则将该点加入 Storage 中
...
void KD_TREE::Push_Down(KD_TREE_NODE *root){
    //* 状态向下传播
    //! 0. 关键在与什么时候一个点会被标记为树删除和树下采样删除,这直接影响了所有子节点的状态
    if (root == nullptr) return;
    Operation_Logger_Type operation;
    ...
    if (root->need_push_down_to_left && root->left_son_ptr != nullptr){
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != root->left_son_ptr){
            //* 1. 如果树跟节点被标记为树下采样删除，则其子节点都会被标记为树下采样删除，点下采样删除和树删除
            ...
            //* 2. 子节点的树下采样删除会传播到子节点的树删除，同样，跟节点的树删除会传到子节点的树删除
            //* 3. 子节点的树删除和树下采样删除都会传播到子节点的点删除
            ...
            //* 4. 如果跟节点被标记为树下采样删除，则子节点的下采样删除点数等于子节点对应的树的点数
            if (root->tree_downsample_deleted)
                root->left_son_ptr->down_del_num = root->left_son_ptr->TreeSize;
            //* 4. 如果跟节点被标记为树删除，则子节点的无效点数等于子节点对应的树的点数，否则无效点数就为下采样删除点数
