...
def stratified_sampling(n_pts, n_samples, distances):
    ...
    interval_dists = interval_dists.unsqueeze(0).repeat(batch_size, n_pts, 1) * distances
    mids = .5 * (interval_dists[..., 1:] + interval_dists[..., :-1]) #* n-1个中间值
    upper = torch.cat([mids, interval_dists[..., -1:]], dim=-1) #* 含尾值
    lower = torch.cat([interval_dists[..., :1], mids], dim=-1) #* 含首值
    # import pdb;pdb.set_trace()
    # stratified samples in those intervals
    t_rand = torch.rand(batch_size, n_pts, n_samples, device=distances.device)
    interval_dists = lower + (upper - lower) * t_rand
    #* 除了首尾值是半个步长，其他都是一个步长，有必要整这么麻烦？
    interval_dists = interval_dists.unsqueeze(-1)
    return interval_dists
...
def hierarchical_sampling(
    ...
    Args:
        n_fine_samples (_type_): _description_ 20
    ...
    #* n_fine_samples 20
    #* n_coarse_samples 15
    #* depths.shape torch.Size([1, 1000]) 点到相机中心的距离
    #* surface.shape torch.Size([1, 1000, 3]) 就是gt_pts
    #* ray_directions.shape torch.Size([1, 1000, 3])
    #* cam_loc tensor([[-1.0949,  0.3687, -2.3160]], device='cuda:0')
    #* offset_distance 0.05
    # import pdb;pdb.set_trace()
    batch_size, n_pts = ray_directions.shape[:2]
    half_distance = torch.zeros_like(depths) + offset_distance # [B, N]
    negative_offset = torch.where(depths < offset_distance, depths, half_distance) #* 满足条件为depths，不满足为half_distance
    start_pts = surface - negative_offset.unsqueeze(-1) * ray_directions
    start_depths = torch.sqrt(torch.sum((start_pts - cam_loc.unsqueeze(1)) ** 2,dim=-1))  # [B, N]
    distances = torch.zeros_like(ray_directions[:, :, :1]) + offset_distance * 2 #* torch.Size([1, 1000, 1])
    # distances = torch.zeros((batch_size, n_pts, 1)).float().cuda() + offset_distance
    #* 划分为多个小区间，在区间内随机取值
    interval_dists_fine = stratified_sampling(n_pts, n_fine_samples, distances) #* torch.Size([1, 1000, 20, 1])
    interval_dists_fine += start_depths.unsqueeze(-1).unsqueeze(-1)
    depths = depths.unsqueeze(-1) #* torch.Size([1, 1000, 1])
    interval_dists_coarse = stratified_sampling(n_pts, n_coarse_samples, depths) #* 这个区间取值就很粗糙了 torch.Size([1, 1000, 15, 1])
    dists, _ = torch.sort(torch.cat([interval_dists_fine, interval_dists_coarse], -2), -2) #* torch.Size([1, 1000, 35, 1])
    pts = cam_loc.unsqueeze(1).unsqueeze(1) + dists * ray_directions.unsqueeze(2)  # [b, N, S, 3]
    # import pdb;pdb.set_trace()
    return pts, dists
...
def render_pts_old(pts, occupied_prob):
    ...
    passthrough_prob = torch.cat([torch.ones_like(passthrough_prob[..., :1]), passthrough_prob],dim=-1)
...
def render_pts_sdf(pts, tsdf, ray_dirs, cam_loc):
    ...
    # [n_batchs, n_pts, 2]
    pad_indices = torch.stack(torch.meshgrid(torch.arange(n_batches, device=pts.device),torch.arange(n_pts, device=pts.device)),dim=-1)
    ...
    tsdf_offset = tsdf[indices[:, 0], indices[:, 1], indices[:, 2]].reshape(n_batches, n_pts)
    closest_pts = pts[indices[:, 0], indices[:, 1], indices[:, 2]].reshape(n_batches, n_pts, 3)
...
def get_camera_params(uv, pose, intrinsics):
    ...
    if pose.shape[1] == 7: #In case of quaternion vector representation
        ...
        cam_loc = pose[:, :3, 3]
        p = pose
    batch_size, num_samples, _ = uv.shape #* torch.Size([1, 1000, 2])
    #* pose.shape torch.Size([1, 4, 4])
    ...
    z_cam = uv[:, :, 0] * 0. + 1.
    #* 归一化坐标
    pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics) #* torch.Size([1, 1000, 4])
...
def render_with_rays(
    ...
    ray_dirs, cam_loc = get_camera_params(uv, T_wcs, intr_mats) #* 返回相机中心到点的方向向量和相机中心坐标
    gt_depths = torch.sqrt(torch.sum((rays['gt_pts'] - cam_loc.unsqueeze(1)) ** 2, dim=-1))  # [v, n_pts]
    pts, dists = hierarchical_sampling( #* 沿着射线采样,精细距离20个，粗糙距离15个，都是世界坐标系下的坐标
        truncated_units*2,
        int(ray_max_dist*5), gt_depths, rays['gt_pts'],
        ray_dirs, cam_loc, offset_distance=truncated_dist,
        max_depth=ray_max_dist
    )
    coords = (pts - volume.min_coords) / volume.voxel_size #* 采样点在网格中的归一化坐标，利用这个坐标找到周围的网格的坐标(整数，索引)
    coords = get_neighbors(coords) #* torch.Size([1, 8, 1000, 35, 3])
    # import pdb;pdb.set_trace()
    volume.count_optim(coords) #* 权重+1
    #* 利用每个点周围的8个网格信息计算每个采样点的sdf
    pred_sdf = volume.decode_pts(pts, nerf, sdf_delta=sdf_delta) #* torch.Size([1, 1000, 35, 1])
    pred_sdf = pred_sdf[..., 0] 
    out = {
        "cam_loc": cam_loc,
        "ray_dirs": ray_dirs,
...
def compute_sdf_loss(
    ...
    gt_depths = torch.sqrt(torch.sum((rays['gt_pts'] - cam_loc.unsqueeze(1)) ** 2,dim=-1)).unsqueeze(-1)  #* [1, n_pts, 1] torch.Size([1, 1000, 1])
    depths = torch.sqrt(torch.sum((pred_pts - cam_loc.unsqueeze(1).unsqueeze(1)) ** 2,dim=-1))  #* [1, n_pts, n_steps] torch.Size([1, 1000, 35])
    # import pdb;pdb.set_trace()
    gt_sdf = gt_depths - depths
    gt_sdf = torch.clip(gt_sdf, min=-truncated_dist, max=truncated_dist) #* 0.05
    valid_map = gt_sdf > max(-truncated_dist*0.5, -0.05) #* -0.025
    #* rays['neighbor_pts'].shape torch.Size([1, 1000, 9, 3]) 
    #* pred_pts.shape torch.Size([1, 1000, 35, 3]) 沿着射线的采样点
    #* 每个采样点到gt周围点的距离，平面3*3的网格对应的周围点，这种方式可以增强鲁棒性
    #* 这个dist相当于射线上每个点的sdf
    dists = torch.sqrt(torch.sum((rays['neighbor_pts'].unsqueeze(2) - pred_pts.unsqueeze(3)) ** 2,dim=-1))  # [1, n_pts, n_steps, n_neighbors]
    #* torch.Size([1, 1000, 35, 9])
    n_samples = pred_pts.shape[2] #* 35
    neighbor_mask = rays['neighbor_masks'].unsqueeze(2).repeat(1, 1, n_samples, 1) # [1, n_pts, n_steps, n_neighbors]
    #* 无效的距离给很大的值
    dists = torch.where(neighbor_mask.bool(), dists, torch.ones_like(dists) * 10000) #* torch.Size([1, 1000, 35, 9])
    # get the corrected SDF using the minimum of a neighborhood
    #* 每个采样点到gt的neighborhood的最近值作为sdf
    gt_nearest_dists = torch.min(dists, dim=-1)[0] #* torch.Size([1, 1000, 35])
    sign = torch.where(gt_sdf > 0, torch.ones_like(gt_sdf), torch.ones_like(gt_sdf) * -1)
    #* 利用真值给这个给距离符号
    gt_nearest_signed_dists = gt_nearest_dists * sign
    gt_nearest_signed_dists = torch.clip(gt_nearest_signed_dists, min=-truncated_dist, max=truncated_dist)
    #* pred_sdf.shape torch.Size([1, 1000, 35])
    depth_bce = F.l1_loss(pred_sdf,gt_nearest_signed_dists,reduction='none') * valid_map
...
def calculate_loss(
    ...
    render_out = render_with_rays(volume, rays, nerf, sdf_delta, truncated_units, truncated_dist, ray_max_dist)
